...
readxl::excel_sheets('abel-database-s2-mod.xlsx')

# choose sheet name and read in its data
data <- read_excel(path = 'abel-database-s2-mod.xlsx', sheet = '2005-10')
...
codes <- read_excel(path = 'abel-database-s1-mod.xlsx', sheet = 'look up')
...
codes_s <- codes %>% select(country, world_region)
...
regflow_long <- regflow1 %>%
  pivot_longer(Aruba:Zimbabwe, names_to = "destination", values_to = "flows")
...
codes_rn <- codes %>% dplyr::rename(dest_region = world_region, destination = country) %>%
...
# SVG graphics device
svg(file="Circular_mig_flow.svg")
